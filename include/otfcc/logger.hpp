...
typedef struct otfcc_ILogger {
	...
	void (*start)(struct otfcc_ILogger *self,
	              const char *segment); // add a level, output a progress
	void (*startSDS)(struct otfcc_ILogger *self,
	                 MOVE sds segment); // add a level, output a progress
	void (*log)(struct otfcc_ILogger *self, uint8_t verbosity, otfcc_LoggerType type,
	            const char *data); // log a data
	void (*logSDS)(struct otfcc_ILogger *self, uint8_t verbosity, otfcc_LoggerType type,
	               MOVE sds data);              // log a data
